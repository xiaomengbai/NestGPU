...
class YExpTool:
    ...
    def convert_token_list_to_exp_tree(self, input_token_list):
        ...
        if len_input_token_list == 3:
        	...
        	#subquery
        	#finally here cause only the T_SELECT is left
        	#do some thing to deal with the T_SELECT
        	if first_token["name"] == "LPAREN" and second_token["name"] == "T_SELECT" and third_token["name"] == "RPAREN":
        		#cons means constant ?
        		return YSubExp(second_token["name"], "SUB")
...
class YFuncExp(YExpBase): #needs modifications for subquery
    ...
    def evaluate(self):
        ...
        ps = ""
        for ap in self.parameter_list: #parameter one is column another is subquery exp
            aps = ap.evaluate()
            ps = ps + str(aps) + ","
...
class YSubExp(YExpBase):
    
    sub_exp_type = None

    sub_select_node = None

    def __init__(self, input_select_node, input_exp_type):

        self.sub_select_node = input_select_node
        self.sub_exp_type = input_exp_type

    def evaluate(self):
        return self.sub_select_node
    ...
    def compare(self,exp):
        ...
        if self.sub_exp_type !=exp.sub_exp_type or self.sub_select_node != exp.sub_select_node:
            return False
...
class SubQNode(QueryPlanTreeBase):
    ...
    subquery_select_node = None
    subquery_position = []
    from_table_list = []
    t0 = None
    t1 = None
    t2 = None
    t3 = None # result of build_plan_tree_from_a_select_node()
...
##
        ...
        elif a_input["type"] == 'SubqueryTable':

            #tn should be a Subquery
            pdb.set_trace()
            tn = SubQNode()
            tn.source = a_input
            tn.i_am_the_tree = i_am_the_tree
            ...
            if i_am_the_tree:
                ...
                tn.having_clause = self.having_clause
                tn.order_by_clause = self.order_by_clause
                tn.subquery_select_node = self.where_condition.subqueryNode
                tn.subquery_position.append('where_condition')
                tn.from_table_list.append(self.from_list[1].content)
            ...
            #Step0: convert subquery select node to plan tree
            tn.subquery_select_node.process_from_list()

            tn.t0 = tn.subquery_select_node.convert_to_initial_query_plan_tree()
            tn.t1 = tn.t0.release_order_by()
            tn.t2 = tn.t1.release_group_by()

            for t in tn.t2.table_list:
				if t != tn.table_name:
					tn.table_list.append(t)

            for t in tn.from_table_list[0:]:
            	tn.t2.table_list.append(t)
            	tn.t2.child.table_list.append(t)
            tn.t3 = tn.t2.convert_to_binary_join_tree()

            tn.routine()
            # should be fine above
...
class FirstStepWhereCondition:
    ...
    def utility_convert_where_condition_item_to_str(self, where_condition_item):
        ...
        #elif where_condition_item.tokenname == 'T_SELECT':
        #    tmp_list = []
        #    converted_select_node = convert_a_select_tree(where_condition_item)
    ...
    def utility_convert_real_where_condition_to_exp(self, real_where_condition):
        ...
        if len(c) == 1:
            ...
            for t_child in c:#.child_list:
                ...
                a_exp_input_list.append(a_token)
                #do some thing to unleash the subqury tokens
                if(t_child.tokenname == "T_SELECT") : # should be fine
                	
                	converted_select_node = convert_a_select_tree(t_child)
                	self.isSubquery = True
                	self.subqueryNode = converted_select_node
                	#for t_t_child in t_child.child_list:
                	#	t_token = {}
                	#	t_token["name"] = t_t_child.tokenname
                	#	t_token["content"] = t_t_child.content
                	#	a_exp_input_list.append(t_token)
...
def build_plan_tree_from_a_select_node(a_query_node):
    ...
    r.process_from_list()
    #process on from list, should be no relation with where condition with subquery    
    ...
    if isinstance(exp,YRawColExp):
        if exp.table_name != "":
            ...
            for x in table_list:
                ...
                if column_in_table(exp.column_name,tmp_name): # C_NATION is ont in TABLE SUPPLIER
                    table_name = tmp_name
                    count = count +1
            if count !=1:
                return -1
...
def ysmart_tree_gen(schema,xml_file):
    ...
    thesubnode = get_the_select_node_from_a_file('./inner.xml')
