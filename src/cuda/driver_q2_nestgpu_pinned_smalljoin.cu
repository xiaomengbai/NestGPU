...
int main(int argc, char ** argv){
    ...
    initTable(partsuppTable);
    {
        ...
        partsuppTable->colIdxNum = 1;
        _partsupp_table->keepInGpuIdx = 1;
        partsuppTable->colIdx = (int *)host_mempool.alloc(sizeof(int) * 1);
        partsuppTable->contentIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        partsuppTable->posIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        partsuppTable->colIdx[0] = 0;
        createIndex(partsuppTable, 0, 0, &pp);
    ...
    initTable(ps0);
    {
        ...
        partsuppTablePartial->colIdxNum = 1;
        partsuppTablePartial->keepInGpuIdx = 1;
        partsuppTablePartial->colIdx = (int *)host_mempool.alloc(sizeof(int) * 1);
        partsuppTablePartial->contentIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        partsuppTablePartial->posIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        partsuppTablePartial->colIdx[0] = 0;
        partsuppTablePartial->posIdx[0] = partsuppTable->posIdx[0];
        partsuppTablePartial->contentIdx[0] = partsuppTable->contentIdx[0];
        partsuppTablePartial->indexPos = partsuppTable->indexPos;
...

    {
        ...
        CUDA_SAFE_CALL_NO_SYNC( cudaMalloc((void **)&correlated_col_sort, partsuppTable->attrTotalSize[0]) );

        if(partsuppTable->dataPos[0] == MEM || partsuppTable->dataPos[0] == PINNED)
        ...
        thrust::lower_bound(thrust::device, (int *)correlated_col_sort, (int *)correlated_col_sort + partsuppTable->tupleNum, (int *)passed_in_col, (int *)passed_in_col + joinRes->tupleNum, right0_lo);
        thrust::upper_bound(thrust::device, (int *)correlated_col_sort, (int *)correlated_col_sort + partsuppTable->tupleNum, (int *)passed_in_col, (int *)passed_in_col + joinRes->tupleNum, right0_hi);
        ...
        if(cache.find(*(int *)_RIGHT_0) == cache.end())
            {
                ...
                initTable(ps1);
                {
                    ...
                    partsuppTablePartial->contentIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
                    partsuppTablePartial->posIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
                    partsuppTablePartial->colIdx[0] = 2;
                    partsuppTablePartial->posIdx[0] = partsuppTable->posIdx[0];
                    partsuppTablePartial->contentIdx[0] = partsuppTable->contentIdx[0];
                    partsuppTablePartial->indexPos = partsuppTable->indexPos;
                    ...
                    if(gpu_inner_mp.freesize() < dev_memsize)
                        gpu_inner_mp.resize(gpu_inner_mp.usedsize() + dev_memsize);
                    char *origin_pos = gpu_inner_mp.freepos();
                    ps1 = tableScan(&partsuppRel, &pp, &host_mempool, &gpu_inner_mp, &gpu_inter_mp, partsupp0_idx, right0_lo + tupleid, right0_hi + tupleid);
                    gpu_inner_mp.freeto(origin_pos);
        ...
        CUDA_SAFE_CALL_NO_SYNC( cudaFree(partsupp0_idx) );
