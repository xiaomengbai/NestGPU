...
int main(int argc, char ** argv){
    ...
    initTable(lineitemTable);
    {
        ...
        lineitemTable->colIdxNum = 1;
        _lineitem_table->keepInGpuIdx = 1;
        lineitemTable->colIdx = (int *)host_mempool.alloc(sizeof(int) * 1);
        lineitemTable->contentIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        lineitemTable->posIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        lineitemTable->colIdx[0] = 1;
    ...
    initTable(li0);
    {
        ...
        lineitemTablePartial->colIdxNum = 1;
        lineitemTablePartial->keepInGpuIdx = 1;
        lineitemTablePartial->colIdx = (int *)host_mempool.alloc(sizeof(int) * 1);
        lineitemTablePartial->contentIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        lineitemTablePartial->posIdx = (int **)host_mempool.alloc(sizeof(int *) * 1);
        lineitemTablePartial->colIdx[0] = 1;
        lineitemTablePartial->posIdx[0] = lineitemTable->posIdx[0];
        lineitemTablePartial->contentIdx[0] = lineitemTable->contentIdx[0];
        lineitemTablePartial->indexPos = lineitemTable->indexPos;
...

    {
        ...
        // Semi-join  RIGHT.0  and  LINEITEM.1
        if(joinRes->dataPos[2] == MEM)
            transferTableColumnToGPU(joinRes, 2);
        passed_in_col = (char *)(joinRes->content[2]);
        correlated_col = lineitemTable->content[1];

        CUDA_SAFE_CALL_NO_SYNC( cudaMalloc((void **)&correlated_col_sort, lineitemTable->attrTotalSize[1]) );

        if(lineitemTable->dataPos[1] == MEM)
            CUDA_SAFE_CALL_NO_SYNC( cudaMemcpy(correlated_col_sort, correlated_col, lineitemTable->attrTotalSize[1], cudaMemcpyHostToDevice) );
        else if(lineitemTable->dataPos[1] == GPU)
            CUDA_SAFE_CALL_NO_SYNC( cudaMemcpy(correlated_col_sort, correlated_col, lineitemTable->attrTotalSize[1], cudaMemcpyDeviceToDevice) );

        int *lineitem1_idx;
        CUDA_SAFE_CALL_NO_SYNC( cudaMalloc((void **)&lineitem1_idx, lineitemTable->tupleNum * sizeof(int)) );
        assign_index<<<2048, 256>>>(lineitem1_idx, lineitemTable->tupleNum);
        thrust::sort_by_key(thrust::device, (int *)correlated_col_sort, (int *)correlated_col_sort + lineitemTable->tupleNum, lineitem1_idx);

        int *right0_lo, *right0_hi;
        CUDA_SAFE_CALL_NO_SYNC( cudaMalloc((void **)&right0_lo, joinRes->tupleNum * sizeof(int)) );
        CUDA_SAFE_CALL_NO_SYNC( cudaMalloc((void **)&right0_hi, joinRes->tupleNum * sizeof(int)) );

        thrust::lower_bound(thrust::device, (int *)correlated_col_sort, (int *)correlated_col_sort + lineitemTable->tupleNum, (int *)passed_in_col, (int *)passed_in_col + joinRes->tupleNum, right0_lo);
        thrust::upper_bound(thrust::device, (int *)correlated_col_sort, (int *)correlated_col_sort + lineitemTable->tupleNum, (int *)passed_in_col, (int *)passed_in_col + joinRes->tupleNum, right0_hi);

        CUDA_SAFE_CALL_NO_SYNC( cudaFree(correlated_col_sort) );
        ...
        if(cache.find(*(int *)_RIGHT_0) == cache.end())
            {
                struct tableNode *result;
                ...
                initTable(li1);
                {
                    ...
                    if(gpu_inner_mp.freesize() < dev_memsize)
                        gpu_inner_mp.resize(gpu_inner_mp.usedsize() + dev_memsize);
                    char *origin_pos = gpu_inner_mp.freepos();
                    li1 = tableScan(&lineitemRel, &pp, &host_mempool, &gpu_inner_mp, &gpu_inter_mp, lineitem1_idx, right0_lo + tupleid, right0_hi + tupleid);
                    gpu_inner_mp.freeto(origin_pos);
...
*(float *)&(((struct mathExp *)                    gbNode->gbExp[0].exp.exp)[1].opValue) = 0.2;
                ...
                memcpy(subqRes0 + tupleid * sizeof(float), &(cache[*(int *)_RIGHT_0]), sizeof(float));
            }
        }
        CUDA_SAFE_CALL_NO_SYNC( cudaFree(lineitem1_idx) );
        CUDA_SAFE_CALL_NO_SYNC( cudaFree(right0_lo) );
        CUDA_SAFE_CALL_NO_SYNC( cudaFree(right0_hi) );
